...
export default function FilterGroupLIst({
  ...
  console.log("CHECK QUERY", filterQueryParams)
  const filterGroupQuery = useInfiniteQuery<FilterByGroupsResponse>(
    [QueryKeys.FILTER_GROUPS, filterQueryParams],
    ({ pageParam }) => {
      return goFetchLite(`groups/filteredGroups` , {
        method: "GET",
  ...
  useEffect(() => {
    if (!filterGroupQuery) return;
    console.log("❌❌❌❌  FILTER GROUP", JSON.stringify(filterGroupQuery.data));
  }, [filterGroupQuery.isLoading]);
  ...
  return (
    ...
    {filterGroupQuery.data?.pages && !filterGroupQuery.isLoading && (
      <ColouredList
        keyPrefix="nearby" 
        onEndReached={() => {
          if (filterGroupQuery.hasNextPage) filterGroupQuery.fetchNextPage();
        }}
        startZIndex={99}
        ListEmptyComponent={() => <EmptyList />}
        items={filterGroupQuery.data.pages.flatMap(
          page =>
          // @ts-expect-error
    ...
    : format(new Date(group.date), "PPPpp").toString() : null}
                      </Text>
                      <Text
                        size="sm"
                        style={{ color: "white", opacity: 0.7 }}
